...
from api.models import CashFlowQuarter
from .http import get_text

YAHOO_CASH_FLOW_URL = "https://tw.stock.yahoo.com/quote/{ticker}/cash-flow-statement"
...
def _extract_row(section) -> list[tuple[str, list[str]]]:
    ...
    for row in section.select("div.table-body-wrapper ul > li"):
        label_cell = row.select_one("div.W\\(120px\\) span")
        if not label_cell:
            continue
...
def fetch_yahoo_cash_flow(ticker: str) -> list[dict]:
    try:
        html = get_text(YAHOO_CASH_FLOW_URL.format(ticker=ticker), timeout=30)
    except requests.HTTPError as exc:
        if exc.response is not None and exc.response.status_code in (404, 429):
            if exc.response.status_code == 404:
                print(f"[warn] yahoo cash flow not found for {ticker}")
            else:
                print(f"[warn] yahoo cash flow rate limited for {ticker}")
            return []
        raise
    except requests.RequestException as exc:
        print(f"[warn] yahoo cash flow request failed for {ticker}: {exc}")
        return []

    soup = BeautifulSoup(html, "lxml")
    section = soup.select_one("section#qsp-cash-flow-statement-table")
    if section is None:
        return []
...
def upsert_yahoo_cash_flow(db: Session, rows: Iterable[dict]) -> int:
    ...
    for row in rows:
        stmt = (
            insert(CashFlowQuarter)
            .values(**row)
            .on_conflict_do_update(
                index_elements=[
                    CashFlowQuarter.ticker,
                    CashFlowQuarter.fiscal_year,
                    CashFlowQuarter.fiscal_quarter,
                ],
                set_={
...
def run_yahoo_cash_flow_sync(ticker: str) -> int:
    ...
    with SessionLocal() as db:
        count = upsert_yahoo_cash_flow(db, rows)
        db.commit()
    return count
