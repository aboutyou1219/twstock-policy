...
from etl.yahoo_income_statement import fetch_yahoo_income_statement, upsert_yahoo_income_statement
...
def top5_crawl(
    ...
    for ticker in tickers:
        try:
            rows = cache.get(ticker) or fetch_yahoo_income_statement(ticker)
        except Exception as exc:
            stats.errors += 1
            print(f"[warn] fetch failed for {ticker}: {exc}")
            rows = []
