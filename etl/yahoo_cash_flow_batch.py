...
from etl.yahoo_cash_flow import fetch_yahoo_cash_flow, upsert_yahoo_cash_flow
...
def _db_top5(db: Session, ticker: str) -> list[tuple]:
    rows = db.execute(
        text(
            """
            SELECT fiscal_year, fiscal_quarter, operating_cash_flow, investing_cash_flow,
...
def full_crawl(
    ...
    for ticker in tickers:
        try:
            rows = fetch_yahoo_cash_flow(ticker)
        ...
        cache[ticker] = rows
        stats.inserted_rows += upsert_yahoo_cash_flow(db, rows)
        stats.processed += 1
        pending += 1
...
def top5_crawl(
    ...
    for ticker in tickers:
        try:
            rows = cache.get(ticker) or fetch_yahoo_cash_flow(ticker)
        except Exception as exc:
            stats.errors += 1
            print(f"[warn] fetch failed for {ticker}: {exc}")
            rows = []
        ...
        stats.inserted_rows += upsert_yahoo_cash_flow(db, top5)
        pending += 1
        if pending >= commit_every:
            db.commit()
            pending = 0
...
def run_single(ticker: str, run_top5: bool) -> BatchStats:
    stats = BatchStats()
    print(f"starting yahoo cash flow batch for {ticker}...")
    with SessionLocal() as db:
        run_id = _start_run(db, endpoint=f"yahoo_cash_flow_batch:{ticker}")
        cache = full_crawl(db, [ticker], stats)
        if run_top5:
            top5_crawl(db, [ticker], cache, stats)
        _finish_run(db, run_id, stats)
        db.commit()
    print("--- Yahoo Cash Flow Batch Report ---")
...
def run(run_all: bool, run_top5: bool) -> BatchStats:
    ...
    stats = BatchStats()
    with SessionLocal() as db:
        run_id = _start_run(db, endpoint="yahoo_cash_flow_batch")
        cache: dict[str, list[dict]] = {}
        if run_all:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Yahoo cash flow batch crawler")
    parser.add_argument("--all", action="store_true", help="crawl all cash flow data")
    parser.add_argument("--top5", action="store_true", help="crawl top5 cash flow only")
    ...
    if args.ticker:
        ...
        print("starting yahoo cash flow batch...")
